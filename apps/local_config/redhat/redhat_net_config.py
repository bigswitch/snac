# Handle redhat interface configuration.  Specifically,
#
# Interfacing with ifconfig/ifup/ifdown
# Generating /etc/sysconfig/network-scrips/ifcfg-*
#
...
class redhat_net_config:
    ...
    def create_new_interfaces_files(self):
        import time
        for interface in self.interfaces:
            if interface.name == 'lo':
                continue
            fd = None
            fn = os.path.join(redhat_net_config.interfaces_dir, 'ifcfg-%s' % interface.name)
            try:
                fd = open(fn, 'w')
            except Exception, e:
                lg.error('Opening %s: %s' % (fn, e))
                return
            fd.write('# Created by SNAC on %s\n' % time.ctime())
            fd.write('DEVICE=%s\n' % interface.name)
            fd.write('ONBOOT=yes\n')
            if interface.dhcp:
                fd.write('BOOTPROTO=dhcp\n')
            else:
                fd.write('BOOTPROTO=none\n')
                fd.write('IPADDR=%s\n' % interface.ip4addr)
                fd.write('NETMASK=%s\n' % interface.ip4mask)
                if interface.ip4gw:
                    fd.write('GATEWAY=%s\n' % interface.ip4gw)
                if interface.ip4dns:
                    fd.write('PEERDNS=yes\n')
                    fd.write('DNS1=%s\n' % interface.ip4dns)
    ...
    def set_interface(self, interface):
        ...
        IfconfigSetProcessProtocol(redhat_net_config.ifconfig_path, interface)
        if interface.ip4gw != '':
            try:
                RouteSetGateway(redhat_net_config.route_cmd, interface)
            except Exception,e :
                print e
    ...
    def fill_gateways(self):                                
        ...
        try:
            ...
            if not os.path.isfile(redhat_net_config.route_table):
                lg.error('Could not locate routing table: '+
                          redhat_net_config.route_table)
                return    
            fd = open(redhat_net_config.route_table)
            if not fd:
    ...
    def init(self):
        ...
        if not os.access(redhat_net_config.ifconfig_path, os.X_OK): 
            config.lg.error("Unable to execute "+redhat_net_config.ifconfig_path+" check permissions?")
            return
        ...
        # read all interfaces
        IfconfigListProcessProtocol(redhat_net_config.ifconfig_path, callback)
        return r
