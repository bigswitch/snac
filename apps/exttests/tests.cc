...
public:
    ...
    void testPython() {
        ...
        n = PyString_FromString("nox.ext.apps.exttests.pytests");
        m = PyImport_Import(n);
        if (PyErr_Occurred()) {
            PyErr_Print();
            PyErr_Clear();
            BOOST_FAIL("Unable to import nox.ext.apps.exttests.pytests.");
        }        
...
private:
    ...
    /*class PyFixture_component_factory
    : public Component_factory {
public:
    PyFixture_component_factory(const Interface_description& i_,
                                PyObject* pyt_)
        : i(i_), pyt(pyt_) { }

    Component* instance(const Context* c, 
                        const json_object* json) const { 
...
private:
    ...
    vector<test_case*> gather_python_tests(list<string> args) {
        ...
        m = PyImport_ImportModule("nox.ext.apps.exttests.unittest");
        if (!m) {
            PyErr_Print();
            PyErr_Clear();
            throw runtime_error("Unable to import nox.ext.apps.exttests.unittest.");
        }        
