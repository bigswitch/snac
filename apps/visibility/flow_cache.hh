...
typedef hash_map<uint64_t, FlowBuf_ptr> FlowBufMap;
typedef hash_map<uint64_t, uint32_t> HostRefCountMap;
...
public:
    ...
    const FlowBuf_ptr get_host_flow_buf(uint64_t host_id, bool create=false);
    const FlowBuf_ptr get_policy_flow_buf(uint32_t policy_id, uint32_t rule_id,
            bool create=false);

    uint32_t get_cur_policy_id() {return cur_policy_id;}

    inline bool get_group_type(uint64_t id, Directory::Group_Type & gtype) const
    {
        return false; //authenticator->get_group_type(id, gtype);
    }
...
private:
    ...
    Disposition handle_host_join_event(const Event& e);
