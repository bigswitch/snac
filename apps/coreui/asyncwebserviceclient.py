...
class AsyncNOXWSClient(NOXWSClient):
    ...
    def _login_ok(self, m):
        # This actually means the login failed.  Should get redirect if
        # succeeded.
    ...
    def _login_err(self, e):
        if e.check(twisted.web.error.Error, twisted.web.error.PageRedirect,
            ...
            if self._request.status == "302":
                # Got redirect that indicates login succeeded
    ...
    def _do_login_request(self):
        ...
        body = urllib.urlencode({ "username": user,
                                  "password": password,
                                  "last_page" : ""})
        self._make_request("POST", self._full_url("/login"), headers, body)
