...
function init_page() {
    ...
    dojo.connect(parentsList, "onDelete", function (item) {
        group.delete_parent(dmws.HostGroup, item.displayName());
    });
    ...
    dojo.connect(subgroupsList, "onDelete", function (item) {
        group.delete_subgroup(dmws.HostGroup, item.displayName());
    });
        ...
        if (principal_group_editable) {
            return { name: "subgroups", header: "Subgroups", 
                     ...
                     editorProps: { group: group, type: "subgroup", title: "Add Subgroup",
                                    ctor : dmws.HostGroup, principal_type: "host" },
    ...
    membersTable = new dojox.grid.DataGrid({
        ...
        region: "center",
        structure: [
            { name: "Host Name", field: "uiMonitorLinkText", width: "25%"},
    ...
    group_util.setup_infopage_edit_hooks("host",group,dmws.HostGroup,
                                         membersTable, membersStore);
    
    var hostgrp_update = coreui.getUpdateMgr().userFnCall({
...
function init_directory() {
    ...
    group = new nox.ext.apps.directory.directorymanagerws.HostGroup({
    ...
    coreui.base.set_nav_title([
        ...
        {
            title_text: "Host Group:",
            nav_text: "Hosts",
            nav_url: "/Monitors/Groups/HostGroups"
        },
        {
...
function fetch_directory() {
    var g = new nox.ext.apps.directory.directorymanagerws.HostGroup({
        initialData: { name: selected_group },
        updateList: ["info" ]
    });
