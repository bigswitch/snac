...
static PyObject *
Radix_search_best2(RadixObject *self, PyObject *args, PyObject *kw_args,
		int inclusive)
{
	radix_node_t *node;
	RadixNodeObject *node_obj;
	...
	if ((node = radix_search_best2(PICKRT(prefix, self), prefix, inclusive))
	    == NULL || node->data == NULL) {
		Deref_Prefix(prefix);
		Py_INCREF(Py_None);
		return Py_None;
...
{
    return Radix_search_best2(self, args, kw_args, 1);
}

PyDoc_STRVAR(Radix_search_containing_doc,
"Radix.search_containing(network[, masklen][, packed] -> None\n\
\n\
Search for the specified network in the radix tree.\n\
\n\
search_containing will return the best (longest) entry that includes the\n\
specified 'prefix', not including an exact match.\n\
\n\
If no match is found, then returns None.");

static PyObject *
Radix_search_containing(RadixObject *self, PyObject *args, PyObject *kw_args)
{
    return Radix_search_best2(self, args, kw_args, 0);
}
...
static PyMethodDef Radix_methods[] = {
	...
	{"search_containing",	(PyCFunction)Radix_search_containing,	METH_VARARGS|METH_KEYWORDS,	Radix_search_containing_doc	},
