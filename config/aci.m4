...
dnl  5=enabled ([yes]/no) (if not given, yes is assumed)
...
AC_DEFUN([ACI_MODULES_VERIFY],[
  ...
  do 
    ...
    if test -z "$aci_mod_bad"; then
      ACI_D("Module $aci_module will be built")
      ACI_D([])
...
AC_DEFUN([ACI_MODULES_SUMMARY],[
  if test "x$aci_modules" != "x"; then
    ...
    do
      ...
      eval value=${MODULES_}${aci_module}
      dnl if test "x$value" != "x" ; then
      dnl   echo "    submodules: $value"
      dnl fi
    done
  else
